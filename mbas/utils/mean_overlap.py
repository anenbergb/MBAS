# heavily copied from monai/metrics/meaniou.py
import torch
...
class MeanOverlap(CumulativeIterationMetric):
    def __init__(
        self,
...
def compute_overlap(
    ...
    """Computes Overlap (Intersection) metric from a batch of predictions.
    ...
    if ignore_empty:
        return torch.where(
            y_o > 0, (intersection) / y_o, torch.tensor(float("nan"), device=y_o.device)
        )
    return torch.where(
        y_o > 0, (intersection) / y_o, torch.tensor(0.0, device=y_o.device)
    )
